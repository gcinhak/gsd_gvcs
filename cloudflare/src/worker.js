...
export default {
    async fetch(req, env, ctx) {
        ...
        try {
            // Popcat — 매크로 사용 사례로 일시 비활성화 (2026-05-26 복귀 예정)
            if (path === '/api/popcat' && req.method === 'GET') {
                return json(
                    { 문경: 0, 음성: 0, 세종: 0, paused: true, message: 'popcat temporarily paused' },
                    200
                );
            }
            if (path === '/api/popcat/increment' && req.method === 'POST') {
                return json(
                    { error: 'popcat temporarily paused', paused: true },
                    503
                );
            }
 ...
 *   - 빈 땅 < CLAIM_UNITS → +STEAL_UNITS to team, -STEAL_UNITS from 리더 (= 0.005% transfer)
 */
